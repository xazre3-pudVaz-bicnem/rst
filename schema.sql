-- ============================================================
-- RST CRM 完全セットアップSQL（冪等・依存順・1回で実行可能）
-- ------------------------------------------------------------
-- Supabase SQL Editor にこのまま全文を貼り付けて「Run」してください。
-- ・何度実行しても壊れません（IF NOT EXISTS / 例外無視）。
-- ・既存データは保持されます。
-- ・初期の5テーブルだけの環境でも、不足テーブル/列だけを安全に追加します。
-- ・外部キーが参照するテーブルを必ず先に作成する順序にしています。
-- ============================================================

-- ============================================================
-- 1. 組織（最上流。他テーブルから参照される）
-- ============================================================
...
-- ============================================================
-- 完了。
-- ・本番のアクセス制御(RLS)を有効化する場合は、別ファイルの
--   migrations/2026-06-26_rls_optional.sql（緩い版） または
--   migrations/2026-06-26_rls_roles.sql（ロール権限＋組織分離）を
--   この後に実行してください（適用前に自分を admin にすること）。
-- ============================================================
