...
export async function runAutoCrawl(admin: any, env: NodeJS.ProcessEnv, opts: CrawlOpts = {}): Promise<any> {
  ...
  if (wantType('sequential')) types.push({ key: 'sequential', type: 'sequential_probe', name: '連番URL探索（全有効ソース）', minMs: 8000, run: async () => {
    ...
    // ドメイン単位で同時数を絞って順番に回すため、所要時間はサイト数に比例する。巡回の時間予算を渡し、
    // 収まらない分は次回に回す（最終探索が古い順なので取り残しは出ない）
    return runAllSequentialProbes(admin, mapsKey, { aiInjectMode: 'standard', autoImportPerRun: 50, ...cfg, ...(master.sequential || {}), probeDailyCap: focused ? (Number(cfg.probeDailyCap) || 900) : 800, ...(focused ? {} : { forwardCount: 20 }), runBudgetMs: innerBudgetMs }, opts.userId || null)
  } })
