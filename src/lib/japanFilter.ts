...
// 海外“出店/オープン”の検出。日本企業が海外に店を出したニュース（＝国内の新店ではない）を弾く。
// 記事に日本の本社住所が抽出されると isForeignAddress では海外と分からないため、
// 「海外地名＋に/へ＋(オープン|出店|進出|上陸|旗艦店…)」という“店舗の所在が海外”を意味する語順で判定する。
// 「フランスで修行したシェフが本庄市にオープン」等の国内店は、海外地名の直後が『に＋開店語』でないため除外されない。
const OVERSEAS_LOC = '台湾|臺灣|台北|中国|中國|上海|北京|広州|深[圳セ]ン?|香港|マカオ|韓国|ソウル|釜山|プサン|シンガポール|タイ王国|バンコク|ベトナム|ハノイ|ホーチミン|マレーシア|クアラルンプール|インドネシア|ジャカルタ|フィリピン|マニラ|ハワイ|ホノルル|グアム|ニューヨーク|ロサンゼルス|サンフランシスコ|米国|アメリカ|カナダ|メキシコ|ブラジル|イギリス|英国|ロンドン|フランス|パリ|ドイツ|ベルリン|イタリア|ミラノ|ローマ|スペイン|マドリード|オランダ|オーストラリア|シドニー|メルボルン|ドバイ|アブダビ|UAE|海外'
// 海外地名の直後が「に/へ/にて」（間に挟めるのは『・台北』のような“地名の補足”だけ）→ その後~8字以内に開店語。
// 直後を厳格化しないと「韓国料理店が大阪市にオープン」の 韓国→(料理店が大阪)市に→オープン のように
// 間の語を跨いで国内店を誤除外してしまう。
const OVERSEAS_OPEN_RE = new RegExp(`(?:${OVERSEAS_LOC})(?:[・･][一-龥ぁ-んァ-ヶA-Za-z]{1,8})?(?:市|州)?\\s?(?:に|へ|にて)\\s?[^。、\\n]{0,8}?(?:出店|オープン|開店|開業|グランドオープン|進出|上陸|[1１一]号店|旗艦店|フラッグシップ|新店舗)`, 'i')
/** 海外への出店・オープンのニュースか（日本企業の海外展開＝国内の新店ではない）。 */
export function detectOverseasOpening(text?: string | null): boolean {
  return OVERSEAS_OPEN_RE.test(String(text || ''))
}
