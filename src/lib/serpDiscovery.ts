...
export async function runSerpDiscovery(admin: any, sourceType: string, mapsKey: string | null, opts: SerpOpts, userId: string | null): Promise<any> {
  ...
  let qualityChecks = 0
  const MAX_QUALITY = 25 // 公式サイトの簡易品質チェック（別ドメインfetch）の上限
      ...
      for (const rr of results) {
        ...
        if (isHp) {
          ...
          if (hasOfficialUrl) {
            ...
            else if (qualityChecks < MAX_QUALITY && (Date.now() - startMs) < budgetMs - 6000) { qualityChecks++; try { ohtml = await fetchPage(official, 7000) } catch { ohtml = '' } }
