  ...
  try {
    for (const q of queries) {
      ...
      for (const rr of results) {
        ...
        const closed = detectNegative(strip(html).slice(0, 3000))
        const d = extractDetail(html)
        // 補完: 電話 or 住所が欠ければ Places/検索で補完（コスト節約のため不足時のみ）
        let enrich: any = null
        const sn0 = sanitizeShopName(d.name, { placesMatched: false })
        const needEnrich = (!d.phone || !d.address) && sn0.valid && !!mapsKey
        if (needEnrich) { try { enrich = await enrichCandidate(mapsKey, { shop: sn0.name, username: '', areaHint: d.address || '', industry: '', havePhone: d.phone || '', haveAddress: d.address || '' }, { maxQueries: 1, perQuery: 4 }) } catch { /* noop */ } }
        ...
        const reason = `${def.label}: 「${rr.title || name}」${closed.closed ? `（${closed.reason}）` : ''}${enrich?.status ? ` / 補完[${enrich.status}]` : ''}`
        const payload: any = {
