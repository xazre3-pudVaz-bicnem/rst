...
// 取得元別の新規性シグナル（新入会員/新規掲載/制作実績/施工事例/開業準備など、新店ワード以外の正当な根拠）。
// これが無いと公式サイト新着・施工事例・商工会議所などが全てHOLDになり投入されないため、汎用の新店根拠に含める。
const BIZ_SIGNAL_RE = /(新規掲載|新規加盟|新入会員|新規会員|入会のお知らせ|新規登録店舗|制作実績|お客様のホームページ|看板が(?:つきました|完成しました)|内装工事中|店舗準備中|開店準備中|プレオープン準備|物件決まりました|オープンに向けて|新装開店|開設いたしました)/i

// 記事/お知らせの公開日を推定（meta/time優先→日本語/スラッシュ日付）。未来日・2年超は無視。最も新しい妥当日を返す。
function extractPublishDate(html: string, bodyStrip: string, nowMs: number): { iso: string | null; daysAgo: number | null } {
  ...
  if (strong.length) { const newest = Math.max(...strong); return { iso: new Date(newest).toISOString().slice(0, 10), daysAgo: Math.floor((nowMs - newest) / 86400000) } }
  // 2) 本文中の日付（YYYY年M月D日 / YYYY/M/D / YYYY-M-D）
  const cand: number[] = []
  const re = /(20\d{2})[年./\-]\s?(\d{1,2})[月./\-]\s?(\d{1,2})/g
  let m: RegExpExecArray | null; let guard = 0
  while ((m = re.exec(bodyStrip)) && guard++ < 60) {
    const t = Date.parse(`${m[1]}-${String(m[2]).padStart(2, '0')}-${String(m[3]).padStart(2, '0')}T00:00:00+09:00`)
    if (inRange(t)) cand.push(t)
  }
  if (!cand.length) return { iso: null, daysAgo: null }
  const newest = Math.max(...cand)
  return { iso: new Date(newest).toISOString().slice(0, 10), daysAgo: Math.floor((nowMs - newest) / 86400000) }
}
